  ...
  return (
    ...
    >
      <div className="w-full md:max-w-none max-w-7xl mx-auto px-4 md:px-8 py-2 md:py-3 flex items-center justify-between h-16 md:h-20">
        {/* Logo */}
        <Link to="/adminPanel" className="flex items-center gap-3 md:gap-2 group transition-transform duration-200 hover:scale-105 md:hover:scale-100 md:transition-none">
          <img
            src={logo}
            alt="buildAble"
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-white/70 font-medium tracking-wide md:hidden">Admin Portal</span>
          </div>
        </Link>

        {/* Desktop Links */}
        <div className="hidden md:flex items-center gap-6 lg:gap-8">
          <LinkItem to="/" icon={LayoutDashboard} label="Home" />
          <LinkItem to="/adminPanel" icon={Users} label="Users" />
          <LinkItem to="/admin/sellers" icon={Store} label="Sellers" />
          <LinkItem to="/profile" icon={Settings} label={user?.name?.split(' ')[0] || "Profile"} />
          <button
            onClick={handleLogout}
            className="flex items-center gap-2 text-white hover:text-blue-200 transition-all duration-200 px-3 py-2 rounded-md hover:bg-[#B84937]/50 font-medium"
          >
            <LogOut size={18} />
            <span>Logout</span>
          </button>
      ...
      >
        <div className="flex flex-col items-stretch px-4 py-4 space-y-1">
          ...
          <LinkItem to="/profile" icon={Settings} label="Settings" />
          <button
            onClick={handleLogout}
