...
import RiderNotificationDropdown from "./RiderNotificationDropdown";
  ...
  return (
    ...
    >
      <div className="w-full md:max-w-none max-w-7xl mx-auto px-4 md:px-8 py-2 md:py-3 flex items-center justify-between h-16 md:h-20">
        {/* Logo */}
        <Link to="/rider/dashboard" className="flex items-center gap-3 md:gap-2 group transition-transform duration-200 hover:scale-105 md:hover:scale-100 md:transition-none">
          <img
            src={logo}
            alt="buildAble"
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-white/70 font-medium tracking-wide md:hidden">Delivery Portal</span>
          </div>
        </Link>

        {/* Desktop Links */}
        <div className="hidden md:flex items-center gap-6 lg:gap-8">
          <LinkItem to="/rider/dashboard" icon={LayoutDashboard} label="Dashboard" />
          <LinkItem to="/profile" icon={Settings} label="Profile" />
        ...
        <div className="flex items-center gap-1 md:hidden">
          ...
          <button
            ...
            className="p-2.5 rounded-lg hover:bg-white/10 transition-all duration-200 active:scale-95"
          >
            {open ? (
              <XIcon size={24} className="stroke-[1.5]" />
            ) : (
